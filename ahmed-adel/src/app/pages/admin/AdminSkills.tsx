...
import { useSite, SkillCategory } from "../../context/SiteContext";
import { skillsAPI } from "../../services/api";

export function AdminSkills() {
  const { skillsList, refreshData } = useSite();
  const [list, setList] = useState<SkillCategory[]>(skillsList);
  const [isSaved, setIsSaved] = useState(false);
  const [isSaving, setIsSaving] = useState(false);

  // Sync list when context data loads/changes (e.g. after page refresh)
  useEffect(() => { setList(skillsList); }, [skillsList]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const item of list) {
        ...
        if (isMongoId) {
          await skillsAPI.update(item._id || item.id, item as unknown as Record<string, unknown>);
        } else {
          await skillsAPI.create(item as unknown as Record<string, unknown>);
        }
      }

      // Delete items that were removed from the list
      const currentIds = new Set(list.map(i => i._id || i.id));
      for (const orig of skillsList) {
        const origId = orig._id || orig.id;
        if (!currentIds.has(origId)) {
          await skillsAPI.delete(origId);
        }
      }
  ...
  const handleEdit = (item: SkillCategory) => {
    ...
    setEditingId(itemId);
    setEditForm({ ...item, skills: [...item.skills] });
  };
  ...
  const handleUpdate = () => {
    ...
    setList(prev => prev.map(item => {
      const itemId = item._id || item.id;
      return itemId === editingId ? { ...item, ...editForm } as SkillCategory : item;
    }));
    setEditingId(null);
    setEditForm({});
  };
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
      <div className="p-6 lg:p-8 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage your competencies and skills.</p>
        </div>
      ...
      <div className="p-6 lg:p-8 space-y-6">
            ...
            <div key={itemId} className="border border-slate-200 dark:border-slate-700 rounded-xl p-5 hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
              {editingId === itemId ? (
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Category Title</label>
                    <input 
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Skills (one per line)</label>
                    <textarea 
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {item.skills.map((skill, sIdx) => (
                        <span key={sIdx} className="px-2.5 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-md text-xs font-medium border border-blue-100 dark:border-blue-800">
                          {skill}
                        </span>
                      ))}
                    </div>
        ...
        {list.length === 0 && (
          <p className="text-center text-slate-500 dark:text-slate-400 py-8">No skill categories found.</p>
        )}
      </div>
    </div>
  );
